...
#for the save team functionfrom django.shortcuts import render, redirect, get_object_or_404
from django.shortcuts import render, redirect, get_object_or_404
from django.http import JsonResponse
from .models import Session, Team, Player, PlayerType
import json
...
def save_team(request, session_id):
    if request.method == 'POST':
        try:
            ...
            # Loop through the posted data to create teams
            for i in range(1, session.number_of_teams + 1):
                team_name = f'Team {i}'
                player_type_name = team_data.get(f'team{i}PlayerType')
                player_names = team_data.get(f'team{i}Players').split(',')

                # Get or create the PlayerType
                player_type, created = PlayerType.objects.get_or_create(name=player_type_name)

                # Create the Team
                team, created = Team.objects.get_or_create(name=team_name, session=session)

                # Add players to the team based on player type
                for player_name in player_names:
                    player, created = Player.objects.get_or_create(name=player_name.strip(), player_type=player_type)
                    team.players.add(player)
            
            return JsonResponse({'status': 'success', 'message': 'Teams saved successfully'})
        except Exception as e:
...
from .models import Session, PlayerType
...
def choice(request, sport_id=None):
    try:
        if sport_id is not None:
            session = get_object_or_404(Session, pk=sport_id)
            session.team_range = range(1, session.number_of_teams + 1)
            player_range = range(1, 12)  # Range for 11 players
            player_types = PlayerType.objects.all()
            return render(request, "choice.html", {"session": session, "player_range": player_range, "player_types": player_types})
        else:
            logger.error("No sport_id provided in URL")
            return render(request, "choice.html", {"session": None})
    except Exception as e:
    ...
    if request.method == 'POST':
        try:
            ...
            for i in range(1, session.number_of_teams + 1):
                team_name = f'Team {i}'
                player_type_name = team_data.get(f'team{i}PlayerType')
                player_names = [team_data.get(f'team{i}Player{j}') for j in range(1, 12)]
...
def team_details(request, session_id):
    session = get_object_or_404(Session, id=session_id)
